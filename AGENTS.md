...
│   ├── Dockerfile
│   ├── config.template.json    # WS 生产模板(唯一模板)
│   ├── entrypoint.sh
│   ├── healthcheck.sh
...
| Task | Location | Notes |
|------|----------|-------|
| Modify proxy config | `docker-image/config.template.json` | WS 模板(唯一;envsubst 需 export 变量,见 Config Templating) |
...
| Local FC deploy | `scripts/deploy-fc.sh` | Reads 根目录 .env(common/deploy/client 三节) |
...
| Compose config | `docker-compose.yml` | 根目录,Local dev(根目录 .env 插值) |
...
`DOCKERHUB_USERNAME`, `DOCKERHUB_TOKEN`, `ALIBABA_CLOUD_ACCOUNT_ID`, `ALIBABA_CLOUD_ACCESS_KEY_ID`, `ALIBABA_CLOUD_ACCESS_KEY_SECRET`, `FC_NODES`, `IMAGE_NAME`. Client 节:`SCENE_NODES`(场景组)。
...
- `config.template.json` uses `${UUID}` / `${WS_PATH}`
